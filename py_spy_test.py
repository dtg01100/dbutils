...
def long_running_string_operations():
    ...
    while time.time() - start_time < 10:  # Run for 10 seconds
        ...
        for i in range(1000):
            dist = edit_distance('USER_TABLE_NAME', 'CUSTOMER_TABLE_NAME')
            dist = edit_distance('ORDER_HISTORY', 'INVOICE_HISTORY') 
            dist = edit_distance('PRODUCT_CATALOG', 'ITEM_CATALOG')
            match = fuzzy_match('CUSTOMER_ORDER_TABLE', 'CUST')
            match = fuzzy_match('USER_PROFILE_INFO', 'USER')
            match = fuzzy_match('PRODUCT_DESCRIPTION', 'PROD')
