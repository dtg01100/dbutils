...
# Read the file
with open('/var/mnt/Disk2/projects/dbutils/src/dbutils/gui/qt_app.py', 'r') as f:
    content = f.read()
...
'''
    ...
    menu_code += '''        # Disconnect options
        if self.connections:
            ...
            for conn in self.connections:
                conn_name = f"Connection to {getattr(conn.source_node if conn.target_node == self else conn.target_node, 'table_name', 'Node')}"
...
for node_name, config in node_configs.items():
    ...
    pattern = rf'(class {node_name}\(.*?\n(?:.*?\n)*?)    def contextMenuEvent\(self, event\):\s*\n        from PySide6\.QtWidgets import QMenu\s*\n        menu = QMenu\(\)\s*\n        delete_action = menu\.addAction\("🗑️ Delete Node"\)\s*\n        delete_action\.triggered\.connect\(self\.delete_node\)\s*\n        menu\.exec\(event\.screenPos\(\)\)\s*\n'
    ...
    if matches:
        ...
        print(f"Warning: Could not find contextMenuEvent for {node_name}")

# Write back
with open('/var/mnt/Disk2/projects/dbutils/src/dbutils/gui/qt_app.py', 'w') as f:
    f.write(content)

print("Done updating context menus!")
