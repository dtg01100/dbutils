...
def test_download_driver_sqlite(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        JDBCDriverDownloader, "_get_download_url_for_version", lambda self, info, version: "http://example.com/test.jar"
    )
...
def test_download_driver_multi_artifact(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "_get_download_url_for_version",
        lambda self, info, version: ["http://example.com/one.jar", "http://example.com/two.jar"],
    )
    out = dl.download_driver("sqlite", version="latest")
