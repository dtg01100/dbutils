...
def test_trie_and_search_index_basic():
    ...
    columns = [ColumnInfo(schema='S', table='FOO_BAR', name='COL1', typename='INT', length=10, scale=0, nulls='N', remarks='x')]
...
async def test_get_all_tables_and_columns_async_query_failure(monkeypatch):
    ...
    monkeypatch.setattr('dbutils.db_browser.query_runner', lambda sql: (_ for _ in ()).throw(RuntimeError('query failed')))
